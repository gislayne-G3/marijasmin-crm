...
export default function PimEditor() {
  ...
  // Estoque por tamanho (todas as cores somadas)
  ...
  // Recarregar dados (sync visual)
  async function recarregar() {
    setStatus('🔄 Recarregando dados do Tiny...')
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 11, color: 'var(--text-muted)', margin: '2px 0 0' }}>
            ...
            {produto.tiny_id && <span style={{ marginLeft: 8, color: '#8e44ad', fontWeight: 600 }}>● Tiny #{produto.tiny_id}</span>}
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '24px 32px 60px' }}>
        ...
        <div style={{
          ...
          {/* Foto */}
          <div style={{ width: 100, height: 130, borderRadius: 8, overflow: 'hidden', flexShrink: 0, background: 'var(--bg)', border: '1px solid var(--border)' }}>
            {fotoP
              ? <img src={fotoP} alt={produto.nome} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ImageOff size={24} color="var(--text-light)" /></div>
            }
          </div>

          {/* Info */}
          <div style={{ flex: 1 }}>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: 'var(--azul)', margin: '0 0 8px' }}>{produto.nome}</h2>
            <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
              <InfoBlock label="Preço Atacado" value={`R$ ${Number(produto.preco_atacado || 0).toFixed(2)}`} color="var(--azul)" />
              <InfoBlock label="Preço Varejo (Nuvemshop)" value={`R$ ${Number(produto.preco_varejo || 0).toFixed(2)}`} color="var(--text-muted)" />
              <InfoBlock label="Categoria" value={produto.categoria} />
              <InfoBlock label="SKU" value={produto.sku || '—'} />
            </div>
          </div>
          ...
          <div style={{
            ...
            display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minWidth: 100,
          }}>
            <span style={{ fontSize: 36, fontWeight: 800, color: stockColor(total), lineHeight: 1 }}>{total}</span>
...
function InfoBlock({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div>
      ...
      <p style={{ fontSize: 14, fontWeight: 700, color: color || 'var(--text)', margin: '2px 0 0', textTransform: 'capitalize' }}>{value}</p>
    </div>
  )
}
